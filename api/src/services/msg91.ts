...
/**
 * Send a login/signup OTP via MSG91's OTP API.
 * The OTP is generated by our server; MSG91 just delivers the SMS.
 * In dev (no AUTH_KEY) the OTP is logged so devs can test without MSG91.
 */
export async function sendLoginOtpSms(
  phoneE164: string,
  otp: string,
  opts?: { otpExpiryMinutes?: number },
): Promise<void> {
  const authkey = process.env.MSG91_AUTH_KEY;
  ...
  if (!authkey) {
    if (process.env.NODE_ENV !== 'production') {
      logger.debug('msg91', `[dev] Login OTP for ${phoneE164}: ${otp}`);
      return;
    }
  ...
  if (!templateId) {
    if (process.env.NODE_ENV !== 'production') {
      logger.debug('msg91', `[dev] Login OTP for ${phoneE164}: ${otp} (no template configured)`);
      return;
    }
    throw codedError(
      'MSG91_OTP_TEMPLATE_ID is not set. Add it to Railway env vars.',
      'MSG91_NOT_CONFIGURED',
    );
  }

  try {
    const result = await sendSimSmsViaOtp(authkey, templateId, mobile, otp, {
      otpLength: otp.length,
      otpExpiryMinutes: opts?.otpExpiryMinutes ?? 10,
    });
    logger.info('msg91', `Login OTP accepted by MSG91 → ${phoneE164}`, {
    ...
    logger.error('msg91', 'Login OTP send failed', { error: msg });
    throw codedError(msg, 'OTP_SEND_FAILED');
  }
}
...
async function sendSimSmsViaOtp(
  ...
  mobile: string,
  code: string,
  opts?: { otpLength?: number; otpExpiryMinutes?: number },
): Promise<{ request_id?: string; status?: string; message?: string }> {
  const otpLength = opts?.otpLength ?? 6;
  const otpExpiryMinutes = opts?.otpExpiryMinutes ?? 10;
  const res = await axios.post(
    OTP_URL,
    {
      authkey,
  ...
  const body = (res.data ?? {}) as Msg91ResponseBody;
  return { request_id: body.request_id, status: body.status, message: body.message };
}
