 ...
 */
export async function sendLoginOtpSms(phoneE164: string, otp: string): Promise<void> {
  const authkey = process.env.MSG91_AUTH_KEY;
  ...
  try {
    await sendSimSmsViaOtp(authkey, templateId, mobile, otp);
    logger.info('msg91', `Login OTP sent → ${phoneE164}`);
  } catch (err: unknown) {
